                    ...
                    switch (event) {
                        ...
                        case Constants.WEBSOCKETEVENT.ORDERUNDOSUCCESS://订单撤销成功的推送
                            if($vm){
                                ...
                                $vm.$notify({
                                    title: $vm.langKey('撤销成功'),
                                    message: $vm.langKey('撤单成功'),
                                    type: 'success'
                                })
                            }
                        ...
                        case Constants.WEBSOCKETEVENT.ORDERUNDOFAILURE://订单撤销失败  订单正在撮合中不能撤消
                            // fqCommon.langKey(langObj,key,options)
                            result=result.data
                            ...
                            if($vm){
                                $vm.$notify({
                                    title: $vm.langKey('订单正在撮合，不支持撤销'),
                                    message: $vm.langKey('订单正在撮合，不支持撤销'),
                                    type: 'warning'
                                })
                            }
                            break
                        ...
                        case Constants.WEBSOCKETEVENT.USERMESSAGE://用户信息推送
                            ...
                            if($vm){
                                $vm.$notify({
                                    title: $vm.langKey('您有新的消息'),
                                    message: $vm.langKey('您有新的消息'),
                                    type: 'success'
                                })
                            }
                        ...
                        case Constants.WEBSOCKETEVENT.RECHARGESUCCESS://用户充值状态 @TODO                            
                            ...
                            if($vm&&result){
                                $vm.$notify({
                                    ...
                                    message: $vm.langKey(result.content),
                                    type: 'success'
                                })
                            }
                            break
